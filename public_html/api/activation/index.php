...
$reply = new stdClass();
	...
	try{
		...
		if($method === "GET"){
			...
			if($profile !== null) {
				...
				if($activation === $profile->getProfileActivationToken()) {
						...
						// throw an exception if the activation token doe not exist
		...
		// update the reply objects status and message state variable if an exception or type exception was thrown
